...
def split_input_text(input_text=''):
    ...
    splits = [word.strip() for word in splits]
...
def predict(pred_config, input_text=''):
    ...
    lines = split_input_text(input_text=input_text)
    ...
    prediction['preds'] = preds_list

    with open('prediction.log', "w", encoding="utf-8") as f:
...
def predict_ner(context: str = ''):
    ...
    preds = predict(pred_config=pred_config, input_text=context)
